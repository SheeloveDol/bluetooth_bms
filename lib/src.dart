...
class Be {
  ...
  static Future<void> readSingleParam(int param) async {
    List<int> payload = [param, 0x0];
    var b = await parameterRead(payload);
    if (b.isNotEmpty) {
      Data.setSingularData(b, param); // always starts at design_cap
    }
  }
  ...
  static batchWrite(Map<int, dynamic> paramsToWite) async {
    ...
    for (var k in paramsToWite.keys) {
      ...
      switch (k) {
        ...
        //   await write(Data.CLOSE_FACTORY_MODE);
        // await readSingleParam(Data.legacy(k));continue;
...
class Data {
  ...
  static bool setSingularData(List<int> batch, int registerResponse) {
    _handleBatchParameterData(batch, registerResponse);
