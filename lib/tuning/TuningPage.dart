...
class _TuningPageStae extends State<TuningPage> {
  ...
  OneInputField cellmV(String title, int index) {
    return OneInputField(text: title, initialValue: Data.cell_mv[index].toStringAsFixed(3), onChange: (v) {});
  }

  OneInputField cellmA(String title, int index) {
    return OneInputField(text: title, initialValue: "cell ma $index", onChange: (v) {});
  }

  OneInputField temp(String title, int index) {
    return OneInputField(text: title, initialValue: Data.ntc_temp[index], onChange: (v) {});
  }
            ...
            child: Column(children: [
                  ...
                  child: PageView(
                      ...
                      children: [
                        TuningSection(
                            ...
                            settingsElements: [for (int i = 0; i < Data.cell_cnt; i++) cellmV("cell${i + 1}", i)]),
                        TuningSection(
                            title: "Modify Cell Amperage",
                            settingsElements: [for (int i = 0; i < Data.cell_cnt; i++) cellmA("cell${i + 1}", i)]),
                        TuningSection(
                            title: "Modify Temperature Sensor",
                            settingsElements: [for (int i = 0; i < Data.ntc_cnt; i++) cellmV("T${i + 1}", i)])
                      ])),
              StatefulBuilder(builder: (BuildContext context, setThisState) {
                ...
                return GroupButton(
                    index: index,
                    onChanged: (int i) {
                      _controller.animateToPage(i, duration: Durations.long4, curve: Easing.standard);
                    });
              })
            ])));
  }
