...
class _ScanPageState extends State<ScanPage> {
  ...
  void onFound(String name, BluetoothDevice device, List<Guid> guids) {
    if (devicesNames.contains(name)) {
      return;
    }
    devicesNames.add(name);
    if (device.advName.length > 1 && guids.contains(Guid("FF00"))) {
  ...
  Widget build(BuildContext context) {
    return SafeArea(
        ...
        child: Stack(children: [
              ...
              child: Container(
                  decoration: const BoxDecoration(
                      gradient: LinearGradient(
                          begin: Alignment.topCenter, end: Alignment.bottomCenter, colors: [Color(0xAE121315), Colors.black]),
          ...
          Positioned(
              bottom: 40, left: (MediaQuery.sizeOf(context).width / 2) - 50, child: Image.asset("assets/logo.png", height: 100))
        ]));
  }
}
